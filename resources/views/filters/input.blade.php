<div class="form-group" id="cktextarea">
    ...
    <div class="col-sm-9 col-lg-10 controls">
        <ul id="myTab1" class="nav nav-tabs">
            ...
            <li class="{{($i++)? '':'active'}}"><a href="#title{{$language->short_code}}" data-toggle="tab">
                    {{$language->title}}</a></li>
            @endforeach
        </ul>
        <div class="tab-content">
            ...
            <div class="tab-pane fade in {{($i++)? '':'active'}}" id="title{{$language->short_code}}">
                <input class="form-control" name="title[{{$language->short_code}}]"
                    value="@if($filters){!! $filters->getTranslation('title',$language->short_code)  !!}@endif"
                    required />
            </div>
            @endforeach
        </div>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-lg-10 controls">
        <input class="form-control" name="link" value="@isset($filters->link){{$filters->link}}@endisset">
    ...
    <div class="col-sm-9 col-md-8 controls">
        <div class="fileupload fileupload-new" data-provides="fileupload">
            <div class="fileupload-new img-thumbnail" style="width: 200px; height: 150px;">
                @if($filters)
                <img src="{{url('uploads/filters/'.$filters->image)}}" alt="" />
                @else
